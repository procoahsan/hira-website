...
const accordionRegex =
  /<[^>]+class="[^"]*elementor-toggle-title[^"]*"[^>]*>(.*?)<\/[^>]+>[\s\S]*?<div[^>]+class="[^"]*elementor-toggle-text-content[^"]*"[^>]*>([\s\S]*?)<\/div>/gi;
const matches = [...html.matchAll(accordionRegex)];

if (matches.length === 0) {
  // Try accordion
  const accRegex =
    /<[^>]+class="[^"]*elementor-accordion-title[^"]*"[^>]*>(.*?)<\/[^>]+>[\s\S]*?<div[^>]+class="[^"]*elementor-accordion-text-content[^"]*"[^>]*>([\s\S]*?)<\/div>/gi;
  ...
  if (matchesAcc.length > 0) {
    ...
    const tabRegex =
      /<[^>]+class="[^"]*elementor-tab-title[^"]*"[^>]*>(.*?)<\/[^>]+>[\s\S]*?<div[^>]+class="[^"]*elementor-tab-content[^"]*"[^>]*>([\s\S]*?)<\/div>/gi;
    const matchesTab = [...html.matchAll(tabRegex)];
    matchesTab.forEach((m) =>
      console.log(
        "Q: " +
          m[1].replace(/<[^>]+>/g, "").trim() +
          "\nA: " +
